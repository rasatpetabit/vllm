...
def sparse_attn_indexer(
    ...
    if has_decode:
        ...
        if current_platform.is_xpu():
            ...
            # (A100/GB10) runs the sharded variant: the decode-shard path is
            # active by default on TP>1, so weights/block_table are the
            # rank-local slices (row_lo:row_hi / group_lo:group_hi), and
            # `max_seq_len` is the compressed-indexer coordinate the topk
            # kernel reads.
